...
const table = new ascii().setHeading('CloudZ Commands','Status');
const top = require(`@top-gg/sdk`);
const vote = new top.Api(config.topggapi);
class CloudZCommands extends EventEmitter {
    constructor(client){
        super();
        this.client = client;
    ...
    loadCommands(){
        ...
        readdirSync(`./src/commands/`).forEach(d => {
            ...
            for(const cmd of commands){
                const CloudZCommand = require(`${process.cwd()}/src/commands/${d}/${cmd}`);
                const command = new CloudZCommand(this.client);
    ...
    async run(message){
        ...
        if(message.content === `<@${this.client.user.id}>`)
        {
            ...
            let embed = new EmbedBuilder().setColor(`#ABBCEF`)
            .setThumbnail(client.user.displayAvatarURL( {dynamic : true }))
            .setAuthor({ name: `${client.user.username}`, iconURL: client.user.displayAvatarURL()})
            .setFooter({text: 'Love From panda_.xd <3'})
  ...
  if (getData) {
    ...
    message.reply(`Welcome Back <@${message.author.id}>, I have removed your AFK `);
        ...
        const CloudZCommand = this.commands.get(commandName) || this.commands.find((c) => c.aliases && c.aliases.includes(commandName));
        if(!CloudZCommand) return;
        const em = new EmbedBuilder().setAuthor({name : `| Command Runned`,iconURL : message.author.displayAvatarURL({dynamic : true})})
                .setDescription(
                    ...
                    **Command ran :** \`${CloudZCommand.name}\`
        ...
        CloudZCommand.run(client,message,args,prefix,player).catch(() => { });
        } catch(e) { console.error(e) } 
    }
}
module.exports = CloudZCommands;